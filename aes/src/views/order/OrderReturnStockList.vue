  ...
  <a-card :bordered="false" :class="'cust-erp-sub-tab'">
    ...
    <div>
      ...
      <a-table ref="table" size="middle" bordered rowKey="id" :scroll="{x:true}" :columns="columns"
        ...
        <template slot="rtQty" slot-scope="text, record,index">
          <editable-cell :text="text" @change="onCellChange(record.id, 'rtQty', $event)"/>
        </template>
        <!--2020-11-13 医院规格信息table的退货原因下拉框-->
        <template slot="rtReason" slot-scope="text, record">
          <j-dict-select-tag style="width: 120px" @input="changeRtReason(record)" v-model="record.rtReason" :disabled="status!=='待提交'"
...
<!--                    <a @click="UpdateOrderReturnStock(record)">保存</a>-->
    ...
    <orderReturnDetail-modal ref="modalForm" @ok="modalFormOk" :refresh="updateList" :mainId="mainId"></orderReturnDetail-modal>
  </a-card>
</template>
...
export default {
  name: 'OrderReturnStockList',
  mixins: [JeecgListMixin],
  components: {EditableCell, OrderReturnDetailModal, JDictSelectTag},
  props: {
    mainId: {
      type: String,
      default: '',
    ...
    return {
      ...
      columns: [
        ...
        // },
        {
          ...
          align: 'center',
          dataIndex: 'rtQty',
          scopedSlots: {customRender: 'rtQty'},
        },
        {
          title: '退货原因',
          align: 'center',
          dataIndex: 'rtReason',
          scopedSlots: {customRender: 'rtReason'},
        },
        // {
        //   title: '药品照片',
        //   align: 'center',
      ...
      url: {
        ...
        list: '/order/orderReturnStock/list',
        edit: '/order/orderReturnStock/edit',
        delete: '/order/orderReturnStock/delete',
        deleteBatch: '/order/orderReturn/deleteBatchOrderReturnDetail',
        exportXlsUrl: '/order/orderReturnStock/exportXls',
        importUrl: '/order/orderReturnStock/importExcel'
      },
      dictOptions: {
        type: [],
        drugType: []
  ...
  methods: {
    ...
    changeRtReason(record) {
      ...
      //获取id所对应的数据
      let rtReason = this.sexDictOptions.find(item => item.value == record.rtReason).text;
      record.rtReasonId = record.rtReason;
      record.rtReason = rtReason;
